...
var numberFormat = function (v, fs, cultureInfo) {
	...
	var doubleSeparator = cultureInfo ? cultureInfo.doubleSeparator : ".";
	...
	for (var i = 0; matches && i < matches.length; i++) {
		...
		if (match === "." && doublePosition < 0) {
			...
			doublePosition = i + match.length - 1;
			continue;
	...
	result += (addDecimalSeparator ? doubleSeparator : "") + resultAfterDecimal;
...
CanvasJS.Chart = (function () {
	...
	Axis.generateValueFormatString = function (range, doublesAt100) {
		var valueFormatString = "#,##0.";
		var numberOfDecimals = doublesAt100;

		if (range < 1) {
			numberOfDecimals += Math.floor(Math.abs(Math.log(range) / Math.LN10));

			if (isNaN(numberOfDecimals) || !isFinite(numberOfDecimals))
				numberOfDecimals = doublesAt100;
		}
		...
		if (!this.valueFormatString) {
			if (this.parent.valueType === "dateTime") {
				...
				if (this.chart.plotInfo.axisPlacement === "xySwapped") {
					doublesAt100 = this.parent.range > 50 ? 0 : (this.chart.width > 500 && this.parent.range < 25) ? 2 : Math.floor(Math.abs(Math.log(this.parent.range) / Math.LN10)) + (this.parent.range < 5 ? 2 : this.parent.range < 10 ? 1 : 0);
					this.valueFormatString = Axis.generateValueFormatString(this.parent.range, doublesAt100);
				}
				else {
					doublesAt100 = this.parent.range > 50 ? 0 : Math.floor(Math.abs(Math.log(this.parent.range) / Math.LN10)) + (this.parent.range < 5 ? 2 : this.parent.range < 10 ? 1 : 0);
					this.valueFormatString = Axis.generateValueFormatString(this.parent.range, doublesAt100);
				}
			}
		}
